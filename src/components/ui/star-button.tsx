...
function StarBackground({ color, clipPathId }: StarBackgroundProps) {
  return (
    <svg
      width="100%"
      height="100%"
...
export function StarButton({
  ...
  return (
    <button
      ...
      ref={pathRef}
      className={cn(
        "relative z-[3] inline-flex h-10 items-center justify-center gap-2 overflow-hidden whitespace-nowrap rounded-3xl px-4 py-2 text-sm font-medium text-white transition-colors disabled:pointer-events-none disabled:opacity-50 group/star-button",
        className
      )}
      {...props}
    >
      ...
      />
      <div
        className="absolute inset-0 z-[1] overflow-hidden rounded-[inherit] border-black/10 text-white dark:border-white/15 dark:text-black"
        style={{ borderWidth: "var(--border-width)" }}
        aria-hidden="true"
      >
        <StarBackground color={backgroundColor} clipPathId={clipPathId} />
      </div>
      <span className="relative z-[5] inline-flex items-center justify-center gap-2">
        {children}
      </span>
    </button>
  );
